...
class ProcessedDataTest(TransactionTestCase):
    ...
    def deactivated_test(self):
        ...
        correx = corrections.candidate_party(
            "WINSTON, ALMA MARIE", "2014", "PRIMARY", "GOVERNOR"
        )
        ...
        for contest in CandidateContest.objects.all():
            ...
            self.assertTrue(
                ...
                msg="Multiple candidacies with same person_id in {}!".format(contest),
            )

            # Confirm there aren't multiple Candidacies with the same filer_id
            filer_id_groups_q = (
                contest.candidacies.filter(
                    person__identifiers__scheme="calaccess_filer_id"
                )
                .values("person__identifiers__identifier")
                .annotate(
                    row_count=models.Count("id"),
                )
                .order_by()
            ...
            self.assertTrue(
                ...
                msg="Multiple candidacies with same filer_id in {}!".format(contest),
            )
