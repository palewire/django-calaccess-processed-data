...
class Command(CalAccessCommand):
    ...
    def load(self):
        ...
        object_list = LookupCodesCd.objects.filter(code_type=16000).exclude(
            code_id=16000
        )
        ...
        for obj in object_list:
            ...
            party, created = Organization.objects.get_or_create(
                name=party_name, classification="party"
            )

            # Keep the code_id too
            party.identifiers.get_or_create(
                scheme="calaccess_lookup_code_id",
                identifier=obj.code_id,
            )

            # If it's not new move on...
            if not created:
                continue

            # Otherwise, log it out
            if self.verbosity > 2:
                self.log(f" Created {party} party")
